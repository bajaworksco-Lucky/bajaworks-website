...
export default function StudioShop() {
  ...
  const studioProducts = products.filter((p: any) => p.division === 'studio' && (active === 'all' || p.category === active));

  const hasOptions = (product: any) => product.options && product.options.length > 1;
  ...
  return (
    <>
      ...
      <div style={{
        ...
        background: 'rgba(242,240,236,0.96)', backdropFilter: 'blur(12px)',
      }}>
        <div style={{
          ...
          {CATEGORIES.map(cat => (
            <button key={cat.id} onClick={() => setActive(cat.id)} style={{
              ...
              color: active === cat.id ? '#111' : 'rgba(17,17,17,0.3)',
      ...
      <div style={{ padding: '48px 0 72px' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 clamp(18px, 4vw, 40px)' }}>
          <div style={{ marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
            <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 32, letterSpacing: 2, margin: 0, color: '#111' }}>
              {CATEGORIES.find(c => c.id === active)?.label || 'All'}
            </h2>
            <span style={{ fontSize: 12.5, color: 'rgba(17,17,17,0.35)' }}>{studioProducts.length} items</span>
          </div>
              ...
              >
                <div style={{
                  ...
                  {product.badge && (
                    <span style={{
                      ...
                      background: product.badge === 'New' ? '#E8753A' : product.badge === 'Popular' ? '#E8753A' : 'rgba(139,109,79,0.8)',
                ...
                <div style={{ padding: '14px 16px 16px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 15, fontWeight: 600, color: '#E8753A' }}>
                      {hasOptions(product) ? 'Starting at ' : ''}${(product.price / 100).toFixed(2)}
                    </span>
                    <span style={{
                      padding: '6px 14px', borderRadius: 4,
                      fontFamily: "'Outfit', sans-serif", fontSize: 11,
                      fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase',
                      background: '#111', color: '#F2F0EC',
                    }}>View</span>
                  </div>
                </div>
              </a>
